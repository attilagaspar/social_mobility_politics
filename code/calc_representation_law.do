...
* name shares in law
merge 1:1 `1' using "$intermediate_data_dir/law_`1'.dta" , gen (merge_law)
...
* GREG SAYS: LET'S ASSUME ELITENESS OF law IS 2%
*gen law_popshare = .005 // ez volt a b calculatorban, én 0.01-gyel dolgoztam


	gen law_popshare  = .005 
...
foreach s in "noble" "roma2"   "top20"  {
	...
	gen relative_rep_`s'_hun = ( `s'_share_in_law / `s'_ip ) / ( hunref_share_in_law  / hunref_ip )
...
foreach s in "noble" "roma2"  "top20"  "hunref"  {
	...
	gen relative_rep_`s' = ( `s'_share_in_law / `s'_ip ) 

}

foreach s in "noble" "roma2" "top20" "hunref"  {

	gen relative_rep_`s'_nr = ( `s'_share_in_law / `s'_ip ) * (1-roma_share_predicted)
...
foreach s in "noble" "roma2"  "top20" "hunref" {
	...
	gen law_share_`s' = ( relative_rep_`s'*law_popshare ) 

}

foreach s in "noble" "roma2" "top20" "hunref"   {

	gen law_share_`s'_nr = ( relative_rep_`s'_nr*law_popshare ) 
...
gen x_elite = invnormal(1-law_share_noble) // elite mean, really y_bar - x_pop
gen x_pop = invnormal(1-law_popshare) // pop mean   // but this is really y_bar -x_pop

gen x_elite_nr = invnormal(1-law_share_noble_nr) // elite mean, really y_bar - x_pop

foreach s in "noble" "roma2"  "top20" "hunref"  {

	gen x_`s' = invnormal( 1-law_share_`s' ) 

}
foreach s in "noble" "roma2"   "top20" "hunref"  {

	gen x_`s'_nr = invnormal( 1-law_share_`s'_nr ) 
...
preserve 
	...
	save "$intermediate_data_dir/anal_law_`1'_`2'.dta", replace
restore
